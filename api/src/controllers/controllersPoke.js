...
const getPoke = async () => {
    try {
        ...
        const pokeD = await Pokemon.findAll({
          include: {
            model: TypePoke,
            attributes: ['name'],
            through: {
        ...
        console.log('poke db');
        console.log(pokeDB);
        const tips = await Pokemon.findAll({
            include: TypePoke
        });
        //console.log(tips[0].dataValues);
...
const postPoke = async ( pokem ) =>{
    try {
        const {name,image,life,attack,defense,speed,weight,height,type} = pokem;
        ...
        const newPoke = {
            id,
            name:nameb,image,life,attack,defense,speed,weight,height
        }
